(ns my-bank-app.accept-transactions.svg)

(def transaction {:curren-required  [50000 10000]
                  :origin-destiny ["alemania"  "mexico"]
                  :hour 10})
...
(def transaction-validation
  {:curren-required
   ["Please enter an ammount of your current balance" #(not-empty (first %))
    ...
    "your balance is not enough" #(< (first %) (second %))]
   :origin-destiny
   ["please enter your current country" #(not-empty (first %))
    "please enter the transaction's destiny country" #(not-empty (first %))
    "you can't make international transactions"  #(not (= (first %) (second %)))]
   :hour ["it's too late go to sleep" #(< 4 %)]})
...
(defn add-transactions-data
  ...
  (let [transaction-data  (seq [(read-line) (read-line) (read-line) (read-line) (read-line)])
        ...
        my-error-name (validate map-transaction transaction-data)]
